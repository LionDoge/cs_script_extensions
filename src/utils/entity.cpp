...
{
	CGameEntitySystem* entitySystem = GameEntitySystem();
	return addresses::CGameEntitySystem_FindEntityByClassName(entitySystem, pStartEntity, szName);
}
...
{
	CGameEntitySystem* entitySystem = GameEntitySystem();
	return addresses::CGameEntitySystem_FindEntityByName(entitySystem, pStartEntity, szName, pSearchingEntity, pActivator, pCaller, pFilter);
}
...
{
	CGameEntitySystem* entitySystem = GameEntitySystem();
	addresses::CEntitySystem_AddEntityIOEvent(entitySystem, pTarget, pszInput, pActivator, pCaller, &value, flDelay, 0, NULL, NULL);
}
